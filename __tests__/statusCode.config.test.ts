import STATUS_CODE, { TypeStatusCode } from '../src/config/statusCode.config'

describe('statusCode.config.ts', function () {
    it('Should have all HTTP response status code', function () {
        ...
        const data: any = STATUS_CODE;
        Object.keys(data).map((code: any) => {
            expect(data[code].code).toEqual(COMPARING_STATUS_CODE[code].code)
            expect(data[code].description).toEqual(COMPARING_STATUS_CODE[code].description)
        })
    })
})